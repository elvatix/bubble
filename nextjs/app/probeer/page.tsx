...
const benefits = [
  {
    ...
    title: "Hele pool tegelijk",
    desc: "Alle kandidaten in je shortlist tegelijk benaderen met uniek persoonlijke berichten.",
  },
  {
    icon: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
    ),
    title: "Jouw templates",
    desc: "Berichten op basis van jouw eigen tone-of-voice en bewezen templates.",
  },
  {
    icon: (
    ...
    title: "Native in LinkedIn",
    desc: "Volledig geïntegreerd in Recruiter Seat. Geen extra tools of tabbladen.",
  },
  {
    icon: (
    ...
    desc: "Rapportages en inzichten over je outreach performance per recruiter.",
  },
  {
    icon: (
    ...
    title: "GDPR-proof",
    desc: "Volledig AVG/GDPR compliant. Gegevens worden niet opgeslagen buiten jouw sessie.",
  },
  {
    icon: (
    ...
    desc: "Automatische follow-ups die op het juiste moment worden verstuurd.",
  },
];
        ...
        <div className="section-inner px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-[960px] mx-auto">
            {benefits.map((b) => (
              <div
                key={b.title}
                className="bg-white/[0.06] backdrop-blur-sm rounded-2xl border border-white/10 p-6 flex flex-col gap-3"
              >
                <div className="w-11 h-11 rounded-xl bg-elvatix/20 flex items-center justify-center text-elvatix">
                  {b.icon}
                </div>
                <h3 className="text-[15px] font-bold text-white m-0">{b.title}</h3>
                <p className="text-[13px] text-white/60 leading-relaxed m-0">{b.desc}</p>
              </div>
            ))}
          </div>
        </div>
